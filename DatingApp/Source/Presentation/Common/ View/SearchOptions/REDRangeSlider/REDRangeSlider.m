...
- (void)setLeftValue:(CGFloat)leftValue {
    if (leftValue <= (self.minValue - DTAMinAgeValue)) {
...
- (void)layoutSubviews {
    ...
    CGFloat leftValuePercentage = (self.leftValue - self.minValue)/oneHundredPercent;
    ...
    CGFloat rightValuePercentage = (self.rightValue - self.minValue)/oneHundredPercent;
    ...
    self.sliderFillBackground.center = CGPointMake(self.leftHandle.center.x + (self.rightHandle.center.x - self.leftHandle.center.x) / 2.0 , self.sliderBackground.center.y);
}
...
- (void)setupUI {
    ...
    self.leftHandle.image = [UIImage imageWithCGImage:self.leftHandleImage.CGImage scale:1.76f orientation:self.leftHandleImage.imageOrientation];
    ...
    self.rightHandle.image = [UIImage imageWithCGImage:self.rightHandleImage.CGImage scale:1.76f orientation:self.rightHandleImage.imageOrientation];;
...
- (void)leftHandlePanEngadged:(UIGestureRecognizer *)gesture {
    ...
    else if (panGesture.state == UIGestureRecognizerStateCancelled || panGesture.state == UIGestureRecognizerStateEnded || panGesture.state == UIGestureRecognizerStateCancelled) {
