...
- (void)drawRect:(CGRect)rect {
	...
	if (self.loading == NO) {
		...
		if (image) {
			...
			CGRect imageRect = CGRectMake(roundf((self.hudSize.width - imageSize.width) / 2.0f), roundf((self.hudSize.height - imageSize.height) / 2.0f), imageSize.width, imageSize.height);
        ...
        NSDictionary *attributes = @{ NSFontAttributeName: dingbatFont, NSParagraphStyleAttributeName: paragraphStyle };
        
        [dingbat drawInRect:rect withAttributes:attributes];
	}
}

- (void)layoutSubviews {
	self.activityIndicator.frame = CGRectMake(roundf((self.hudSize.width - kIndicatorSize) / 2.0f), roundf((self.hudSize.height - kIndicatorSize) / 2.0f), kIndicatorSize, kIndicatorSize);
...
- (void)show {
	...
	CGRect contentFrame = CGRectMake(roundf((windowSize.width - self.hudSize.width) / 2.0f), roundf((windowSize.height - self.hudSize.height) / 2.0f) + 10.0f, self.hudSize.width, self.hudSize.height);
